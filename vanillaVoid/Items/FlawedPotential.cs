...
{
    public class FlawedPotential : ItemBase<FlawedPotential>
    {
        public ConfigEntry<float> baseDamageBuff;

        public ConfigEntry<float> stackingBuff;

        public override string ItemName => "Flawed Potential";

        public override string ItemLangTokenName => "FLAWEDPEARL_ITEM";
        ...
        public override void CreateConfig(ConfigFile config)
        {
            ...
            voidPair = config.Bind<string>("Item: " + ItemName, "Item to Corrupt", "Pearl, ShinyPearl", "Adjust which items this is the void pair of.");
        }
            ...
            rules.Add("mdlExecutioner", new RoR2.ItemDisplayRule[]
{
        ...
        public override void Hooks()
        {
            ...
            RecalculateStatsAPI.GetStatCoefficients += FlawedPotentialRecalc;
        }

        private void FlawedPotentialRecalc(CharacterBody sender, RecalculateStatsAPI.StatHookEventArgs args)
        {
            if (sender.inventory.GetItemCount(this.ItemDef) > 0)
            {
                args.baseCurseAdd += .01f;
                args.baseShieldAdd += sender.maxHealth * .1f;
                args.critDamageMultAdd += .1f;
                args.cooldownMultAdd += .1f;
                args.sprintSpeedAdd += .1f;
            }
        }
