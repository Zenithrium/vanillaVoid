...
namespace vanillaVoid.Items
{
    public class LensOrrery : ItemBase<LensOrrery>
    {
        ...
        public override string ItemPickupDesc => "Critical Strikes can occur an additional time, with half the chance of the previous one. Lens-related items are more effective. <style=cIsVoid>Corrupts all Laser Scopes</style>.";
        ...
        public override string ItemFullDescription => $"Gain <style=cIsDamage>{baseCrit.Value}% critical chance</style>. Lens-Maker's Glasses and Lost Seer's Lenses are <style=cIsUtility>{newLensBonus.Value * 100}%</style> <style=cIsUtility>more effective</style>. <style=cIsDamage>Critical strikes</style> can occur <style=cIsDamage>{additionalCritLevels.Value}</style> <style=cStack>(+{additionalCritLevels.Value} per stack)</style> additional times, with each dip having {critModifier.Value * 100}% (+{critModifierStacking.Value * 100}% per stack) of the crit chance of the previous crit. <style=cIsVoid>Corrupts all Laser Scopes</style>.";
        ...
        public override void CreateConfig(ConfigFile config)
        {
            ...
            newStackingLensBonus = config.Bind<float>("Item: " + name, "Glasses Buff per Stack", 0f, "Adjust the percent buff to crit glasses per stack. This won't show up in the tooltip if enabled, though.");
            ...
            critModifierStacking = config.Bind<float>("Item: " + name, "Crit Reduction Reduction", .05f, "Adjust how much the crit reduction is reduced per stack. I don't know how to phrase this better");
