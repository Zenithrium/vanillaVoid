...
{
    public class UrchinTide : ItemBase<UrchinTide>
    {
        public ConfigEntry<float> baseDamageBuff;

        public ConfigEntry<float> stackingBuff;

        public override string ItemName => "Urchin Tide";

        public override string ItemLangTokenName => "URCHIN_ITEM";

        public override string ItemPickupDesc => $"Chance on hit to apply Cascade, damaging enemies over time. When Cascade ends, it spreads to nearby enemies. <style=cIsVoid>Corrupts all {"{CORRUPTION}"}</style>.";
