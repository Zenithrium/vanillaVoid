...
{
    public class Bees : ItemBase<Bees>
    {
        ...
        public override string ItemLore => $"They're everywhere.";
        ...
        public override void Init(ConfigFile config)
        {
            ...
            beesAura = PrefabAPI.InstantiateClone(beesAuraTemp, "beesAura");
        ...
        public void CreateBuff()
        {
            ...
            beesActive.name = "ZnVV" + "beesActive";
        ...
        {
            baseDamage = config.Bind<float>("Item: " + ItemName, "Base Percent Damage", 1.6f, "Adjust the percent damage dealt to each target when active.");
            stackingDamage = config.Bind<float>("Item: " + ItemName, "Stacking Percent Damage", 0, "Adjust the percent damage dealt to each target when active.");
            ...
            baseRange = config.Bind<float>("Item: " + ItemName, "Base Range", 20, "Adjust the base range of the item.");
            stackingRange = config.Bind<float>("Item: " + ItemName, "Stacking Range", 0, "Adjust the range gained per stack.");

            baseDuration = config.Bind<float>("Item: " + ItemName, "Base Duration", 3, "Adjust the duration the item remains active without any targets.");
            stackingDuration = config.Bind<float>("Item: " + ItemName, "Stacking Duration", 0, "Adjust the additional duration gained per stack that the item remains active without any targets.");

            frequency = config.Bind<float>("Item: " + ItemName, "Hit Frequency", .3f, "Adjust how often the item hits enemies per second.");
            ...
            rules.Add("mdlExecutioner", new RoR2.ItemDisplayRule[]
{
            ...
            if (self && self.body && self.body.inventory){ //hit refresh/activate
                ...
                if (stackCount > 0){
                    ...
                    if (!token){
                        ...
                        var range = ItemBase<Bees>.instance.baseRange.Value + (ItemBase<Bees>.instance.stackingRange.Value * (stackCount - 1));
    ...
    public class BeesToken : MonoBehaviour
    {
        ...
        public void FixedUpdate()
        {
            ...
            if (timer >= ItemBase<Bees>.instance.frequency.Value / (ItemBase<Bees>.instance.atkScale.Value ? body.attackSpeed : 1))
            {
                ...
                HurtBox[] hurtBoxes = new SphereSearch{
                    origin = pos,
                    radius = ItemBase<Bees>.instance.baseRange.Value + (ItemBase<Bees>.instance.stackingRange.Value * (itemCount - 1)),
                ...
                int totalHits = ItemBase<Bees>.instance.baseTargets.Value + (ItemBase<Bees>.instance.stackingTargets.Value * (itemCount - 1));
                ...
                for (int i = 0; i < totalHits; ++i)
                {
                    ...
                    lightningOrb.damageValue = ItemBase<Bees>.instance.baseDamage.Value + (ItemBase<Bees>.instance.stackingDamage.Value * (itemCount - 1));
                    ...
                    lightningOrb.procCoefficient = (body.isPlayerControlled ? ItemBase<Bees>.instance.procCoeff.Value : 0);
