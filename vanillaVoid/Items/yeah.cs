...
{
    public class Yeah : ItemBase<Yeah>
    {
        public ConfigEntry<float> baseDamageBuff;

        public ConfigEntry<float> stackingBuff;

        public override string ItemName => "Yeah";

        public override string ItemLangTokenName => "ADZE_ITEM2";

        public override string ItemPickupDesc => $"Gain a portion of prevented damage as barrier. <style=cIsVoid>Corrupts all {"{CORRUPTION}"}</style>.";
                ...
                if (self.body.inventory)
                {
                    ...
                    if (stackCount > 0)
                    {
                        ...
                        if (damageInfo.rejected && self.body.isPlayerControlled)
                        {
                            ...
                            Debug.Log("damge rejected;l " + redu);

                        }
                        else
                        {
                            Debug.Log("yeag: " + self.body.armor + " | " + damageInfo.damage + " | " + self.health);
                            redu = armor / (armor + 100f);
                            Debug.Log("redu mult: " + redu);
                            redu *= damageInfo.damage;
                            Debug.Log("damage redu: " + redu);
                        ...
                        Debug.Log("damage reduction: " + redu + " | " + redu * .25f + " | " + plateRedu);


                        self.AddBarrierAuthority((redu + plateRedu) * .25f);
